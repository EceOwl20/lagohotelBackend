// app/[locale]/panel/sayfalar/hakkimizda/page.jsx
"use client";
...
export default function AboutPanelPage() {
  ...
  const [syncedLang, setSyncedLang] = useState("tr");     
  ...
  const sectionDefs = useMemo(
    () => [
      ...
      {
        id: "info-section",
        title: "💆 Info Section (SpaTypes tarzı)",
        render: () => (
          <SpaTypesInfoSectionEdit
            data={data}
            setData={setData}
            blockName="infoSection"
            langs={LANGS}
            activeLang={activeLang}
          />
        ),
  ...
  // Dil değişimini localStorage'a yaz + hydration-safe olarak senkronize et
  useEffect(() => {
    if (typeof window === "undefined") return;
    const stored = localStorage.getItem("panel_lang");
    if (stored) {
      setActiveLang(stored);
      setSyncedLang(stored);
    }
  }, []);
  ...
  }, [data]); // en güncel data'yı kaydetsin
  ...
  return (
    <header id="top" className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b">
      <div className="max-w-7xl mx-auto px-4 lg:px-6 h-[72px] flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {/* Dil sekmeleri (hydration-safe: activeLang UI için senkronize edilmiş değer) */}
