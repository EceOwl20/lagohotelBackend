...
export default function PoolListEdit({ data, setData, langs }) {
  ...
  const addPool = () =>
    setData({ ...data, poolList: [...arr, { image: "", hoverImage: "", subtitle: {}, title: {}, description: {} }] });
  const removePool = (idx) =>
    setData({ ...data, poolList: arr.filter((_, i) => i !== idx) });

  return (
    <div className="mb-8">
      <h3 className="font-bold text-lg mb-2">Havuzlar</h3>
      <button type="button" className="mb-3 px-4 py-1 bg-green-600 text-white rounded" onClick={addPool}>+ Havuz Ekle</button>
      {arr.map((item, idx) => (
        <div key={idx} className="border rounded p-3 mb-4 bg-gray-50">
          <button type="button" className="mb-2 px-3 py-1 bg-red-500 text-white rounded" onClick={() => removePool(idx)}>Sil</button>
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <input type="file" accept="image/*" onChange={e => uploadImage(e, idx, "image")} disabled={uploading[`${idx}-image`]} />
            {uploading[`${idx}-image`] && <span className="text-blue-500">Yükleniyor...</span>}
          </div>
          <label className="block font-semibold mb-2">Hover Görsel</label>
          <div className="flex items-center gap-4 mb-2">
            {item.hoverImage && (
              <img src={`http://localhost:5001${item.hoverImage}`} alt="" className="w-[90px] h-[60px] object-cover rounded" />
            )}
            <input type="file" accept="image/*" onChange={e => uploadImage(e, idx, "hoverImage")} disabled={uploading[`${idx}-hoverImage`]} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {langs.map(({ key, label }) => (
              <div key={key} className="bg-white rounded p-2">
                ...
                <input type="text" value={item.subtitle?.[key] || ""} onChange={e => handleChange(idx, "subtitle", key, e.target.value)} className="border rounded p-2 w-full mb-1" />
                ...
                <textarea value={item.description?.[key] || ""} onChange={e => handleChange(idx, "description", key, e.target.value)} className="border rounded p-2 w-full mb-1" />
              </div>
            ))}
          </div>
        </div>
