...
import gallery1 from "./images/img-2.jpg"
...
const page = () => {
  ...
  const t = useTranslations('Abellapatisserie');
  const t2 = useTranslations('Abellapatisserie.ClinaryInfoSection');
  const t3 = useTranslations('Abellapatisserie.RoomTour');
  const t4 = useTranslations('Abellapatisserie.CuisinesCarousel');
  const t5 = useTranslations('Abellapatisserie.DiscoverBackground');
        ...
        const fetchData = async () => {
          try {
            const res = await fetch(
              `${apiUrl}/api/pages/barcafes/subbarcafes/abellapatisserie?lang=${locale}`
            );
