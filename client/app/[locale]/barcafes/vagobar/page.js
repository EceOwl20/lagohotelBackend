...
import mainImg from "./images/img-2.jpg"
...
const page = () => {
  const t = useTranslations('VagoBar');
  const t2 = useTranslations('VagoBar.ClinaryInfoSection');
  const t3 = useTranslations('VagoBar.RoomTour');
  const t4 = useTranslations('VagoBar.CuisinesCarousel');
  const t5 = useTranslations('VagoBar.DiscoverBackground');
      ...
      const fetchData = async () => {
        try {
          const res = await fetch(
            `${apiUrl}/api/pages/barcafes/subbarcafes/vagobar?lang=${locale}`
          );
